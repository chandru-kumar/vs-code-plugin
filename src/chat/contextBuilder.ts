...
function buildSystemPrompt(): string {
  ...
  return [
    ...
    '4. EDIT — choose the right tool:',
    '   • To change existing code, FIRST read it with read_file_range (or use',
    '     document_outline) to learn the exact line numbers, then call',
    '     replace_lines(path, startLine, endLine, newText). This is the most',
    '     reliable editor — it touches only those lines and preserves the rest.',
    '   • apply_diff (exact-text match) is fine for a tiny, obviously-unique edit.',
    '   • write_file is ONLY for creating a NEW file. It will REFUSE to overwrite',
    '     an existing file unless you pass overwrite:true with the COMPLETE file —',
    '     never pass a fragment to write_file (it would delete the rest of the file).',
    '   Edits are STAGED and shown to the user as a reviewable diff — you do not',
    '   need to ask permission, but never claim a change is applied; say it is',
    '   proposed for review.',
    ...
    '   (read the file to get exact lines/text) or switch tool (replace_lines).',
    '',
